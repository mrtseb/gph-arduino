...
uses
  ...
  Dialogs, Menus, StdCtrls, ExtCtrls, tabassoc, ShockwaveFlashObjects_1_0_TLB,
  activexcontainer, ComCtrls;
const
   max=50;
   max_char=72;
...
self.flash1.oleserver.Loop:=true;
self.flash1.oleserver.Play;
for i:=0 to step_count do begin
cmd:=trim(uppercase(self.ordres[i]));
if pos('F1V',cmd) <> 0 then self.flash1.oleserver.SetVariable('Feu1','V');
if pos('F1R',cmd) <> 0 then self.flash1.oleserver.SetVariable('Feu1','R');
if pos('F10',cmd) <> 0 then self.flash1.oleserver.SetVariable('Feu1','O');
if pos('F2V',cmd) <> 0 then self.flash1.oleserver.SetVariable('Feu2','V');
if pos('F2R',cmd) <> 0 then self.flash1.oleserver.SetVariable('Feu2','R');
if pos('F20',cmd) <> 0 then self.flash1.oleserver.SetVariable('Feu2','O');
if pos('F3V',cmd) <> 0 then self.flash1.oleserver.SetVariable('Feu3','V');
if pos('F3R',cmd) <> 0 then self.flash1.oleserver.SetVariable('Feu3','R');
if pos('F30',cmd) <> 0 then self.flash1.oleserver.SetVariable('Feu3','O');
//flash1.GotoFrame(1);
//flash1.GotoFrame(0);
end;
exit;
...
if dico.EstPresent('inter1') then begin self.flash1.oleserver.SetVariable('Inter1',dico['inter1']); self.flash1.oleserver.Play;  end;
if dico.EstPresent('inter0') then begin self.flash1.oleserver.SetVariable('Inter0',dico['inter0']); self.flash1.oleserver.Play;  end;
if dico.Keys.Count=0 then
begin
self.flash1.oleserver.SetVariable('Inter0','0');
self.flash1.oleserver.Play;
self.flash1.oleserver.SetVariable('Inter1','0');
self.flash1.oleserver.Play;
dico['inter0']:='0';
dico['inter1']:='0';
end;
...
self.flash1.oleserver.SetVariable('Lampe','0');
self.flash1.oleserver.Play;
if pos('ALLUMER',cmd) <> 0 then begin self.flash1.oleserver.setVariable('Lampe','1'); self.flash1.oleserver.Play;  end;
if pos('ETEINDRE',cmd) <> 0 then begin self.flash1.oleserver.setVariable('Lampe','0'); self.flash1.oleserver.Play; end;
end;
exit;
end;
...
self.flash1.oleserver.Loop:=true;
for i:=0 to step_count do begin
if dico.EstPresent('inter') then begin self.flash1.oleserver.SetVariable('Inter',dico['inter']); self.flash1.oleserver.Play;  end;
cmd:=trim(uppercase(self.ordres[i]));
if (cmd='') then continue;
self.flash1.oleserver.SetVariable('Lampe','0');
self.flash1.oleserver.Play;
    ...
    for i:=0 to step_count do begin
      ...
      if pos('MONTER',cmd) <> 0 then
         begin
          ...
          self.flash1.oleserver.GotoFrame(self.frame);
         end;
...
var i,j:integer;
begin
axc.ComServer:=flash1.ComServer;
axc.Active:=true;
axc.Visible:=true;
form1.picaxe:='18m2';
...
var i,j:integer;
begin
self.flash1.oleserver.GotoFrame(0);
timer1.Enabled:=false;
...
if self.OpenDialog1.Execute then begin
   ...
   self.caption:=s;
   if flash<>'' then begin
      self.flash1.oleserver.movie:=ExtractFilePath(Application.exename)+'\GPH\'+s;
      self.flash1.Visible:=true;
      self.flash1.oleserver.Loop:=false;
